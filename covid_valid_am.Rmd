...
# Фильтруем тех, у кого исход наступил менее чем за три дня:
...
```{r}
data_p <- data[, which(colMeans(!is.na(data)) > 0.3)]
colnames(data_p) <- gsub(' ','_', colnames(data_p))
colnames(data_p) <- gsub('-','_', colnames(data_p))
colnames(data_p) <- gsub('[()]','', colnames(data_p))
# for(i in 6:40) {
#   arg <- colnames(data_p)[i]
#   p <- ggplot(data_p, aes_string(x = 'DaysBfOutcome', y = arg, fill = 'Outcome')) + 
#   geom_boxplot() +  ylab(arg)
#   name <- capture.output(cat("plot_", arg,".png", sep = '' ))
#   ggsave(name)
# }


colnames(data_p)
arg <- colnames(data_p)[7]
ggplot(data_p, aes_string(x = 'DaysBfOutcome', y = arg, fill = 'Outcome')) + 
  geom_boxplot() +  ylab(arg)
ggplot(data, aes(x = DaysBfOutcome, y = `aptt`, fill = Outcome)) + 
  geom_boxplot()
